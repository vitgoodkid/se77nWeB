  ...
  return (
    <div className="ks-modal ks-modal--editor" role="dialog" aria-modal="true" aria-label="Sửa sản phẩm"><button className="ks-modal__backdrop" type="button" onClick={onClose} aria-label="Đóng" /><form className="ks-editor" onSubmit={save}>
      ...
      <div className="ks-editor__grid"><section>
        ...
        {draft.variants.map((variant, index) => <div className="ks-variant-editor" key={`${variant.id}-${index}`}><div className="ks-variant-editor__head"><span style={{ background: variant.colorHex }} /><strong>Mẫu {index + 1}</strong>{draft.variants.length > 1 && <button type="button" onClick={() => setDraft((current) => ({ ...current, variants: current.variants.filter((_, variantIndex) => variantIndex !== index) }))}>Xoá</button>}</div><div className="ks-editor__row"><label><span>Tên mẫu</span><input value={variant.name} onChange={(event) => patchVariant(index, 'name', event.target.value)} /></label><label><span>Màu</span><input type="color" value={variant.colorHex} onChange={(event) => patchVariant(index, 'colorHex', event.target.value)} /></label></div><label><span>URL ảnh riêng</span><input value={variant.imageUrl || ''} onChange={(event) => patchVariant(index, 'imageUrl', event.target.value)} /></label><label className="ks-file ks-file--small"><input type="file" accept="image/png,image/jpeg,image/webp" onChange={(event) => pickImage(event.target.files?.[0], index)} /><strong>Tải ảnh mẫu lên</strong></label><div className="ks-stock-grid">{sizes.map((size) => <label key={size}><span>{size}</span><input type="number" min="0" value={variant.stockBySize?.[size] || 0} onChange={(event) => patchStock(index, size, event.target.value)} /></label>)}</div></div>)}
      </section></div>
