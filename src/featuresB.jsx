...
// ═════════════════════════════════════════════════════════════
// 9. KATAS DASHBOARD — launcher for /kata static mockups
// ═════════════════════════════════════════════════════════════
const KATA_PAGES = [
  { id: 'index',       short: '01', label: 'landing · public',     path: '/kata',                   accent: COLORS.red },
  { id: 'servers',     short: '02', label: 'servers list',         path: '/kata/servers.html',      accent: COLORS.green },
  { id: 'overview',    short: '03', label: 'server overview',      path: '/kata/overview.html',     accent: COLORS.red },
  { id: 'logs-image',  short: '04', label: 'logs · image',         path: '/kata/logs-image.html',   accent: COLORS.gold },
  { id: 'cost',        short: '05', label: 'cost · dark',          path: '/kata/cost.html',         accent: COLORS.gold },
  { id: 'cost-light',  short: '06', label: 'cost · light variant', path: '/kata/cost-light.html',   accent: COLORS.gold },
  { id: 'mobile',      short: '07', label: 'mobile · 6 screens',   path: '/kata/mobile.html',       accent: COLORS.green },
];

export function KataDashboard() {
  function open(path) {
    window.open(path, '_blank', 'noopener');
  }
  return (
    <div style={{ height: '100%', overflowY: 'auto', paddingRight: 8 }}>
      <Panel style={{ padding: 28, marginBottom: 18 }}>
        <Kicker accent={COLORS.red}>09 · KATAS DASHBOARD</Kicker>
        <div className="mono" style={{
          ...
          se<span style={{
            color: COLORS.red,
            textShadow: '0 0 6px ' + COLORS.red + 'cc, 0 0 18px ' + COLORS.red + '66',
          }}>77</span>n
          <span style={{ color: COLORS.muted, fontWeight: 400 }}> / kata</span>
        </div>
        <div style={{ color: COLORS.muted, fontSize: 13, lineHeight: 1.6, maxWidth: 620, marginTop: 14 }}>
          Web dashboard quản lí bot Discord <span style={{ color: COLORS.text }}>KataS</span>.
          Bot host: Sparkedhost · Domain: <span className="mono" style={{ color: COLORS.text }}>se77n.com/kata</span>.
          Hiện đang ở giai đoạn mockup tĩnh (Tailwind CDN) — sẽ rewrite Next.js + WS realtime + MongoDB.
        </div>
        <div style={{ display: 'flex', gap: 10, marginTop: 18, flexWrap: 'wrap' }}>
          <Btn variant="solid" color={COLORS.red} onClick={() => open('/kata')}>
            Mở dashboard ↗
          </Btn>
          <Btn onClick={() => open('/kata/_index.html')}>Mockup index ↗</Btn>
        </div>
      </Panel>

      <Kicker style={{ marginBottom: 12 }}>─── 7 trang mockup ─────────────────────────</Kicker>
      <div style={{
        ...
        {KATA_PAGES.map((p) => (
          <button
            ...
            onMouseEnter={(e) => { e.currentTarget.style.borderColor = COLORS.line2 || 'rgba(255,255,255,0.18)'; e.currentTarget.style.transform = 'translateY(-2px)'; }}
            onMouseLeave={(e) => { e.currentTarget.style.borderColor = COLORS.line; e.currentTarget.style.transform = 'none'; }}
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 6 }}>
              <span className="mono" style={{ fontSize: 10, letterSpacing: '0.18em', textTransform: 'uppercase', color: p.accent }}>{p.short}</span>
              <span className="mono" style={{ fontSize: 10, letterSpacing: '0.18em', textTransform: 'uppercase', color: COLORS.muted }}>↗</span>
            </div>
            <div style={{ fontSize: 17, fontWeight: 500, letterSpacing: '-0.01em', textTransform: 'lowercase' }}>
              {p.label}
            </div>
            <div className="mono" style={{ fontSize: 10, color: COLORS.muted, marginTop: 8 }}>
              {p.path}
            </div>
          </button>
        ))}
